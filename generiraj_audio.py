"""
Generira MP3 audio datoteke za sve eksponante na svim jezicima.
Koristi Microsoft Edge TTS (besplatno, neuralni glasovi).
Pokreni: python generiraj_audio.py
"""
import asyncio
import json
import os
...
async def main():
    ...
    for e in eksponati:
        ...
        # Kratki (easy_to_read_opis)
        hr_tekst = e.get("easy_to_read_opis") or e.get("kratki_opis") or e.get("naziv", "")
        zadaci.append((hr_tekst, GLASOVI["hr"], os.path.join(AUDIO_DIR, f"{eid}-hr.mp3")))
        ...
        for lang_key, glas in [("en", GLASOVI["en"]), ("cz", GLASOVI["cz"])]:
            ...
            if not p:
                continue
            # Kratki
            tekst = p.get("easy_to_read_opis") or p.get("kratki_opis") or p.get("naziv", "")
            zadaci.append((tekst, glas, os.path.join(AUDIO_DIR, f"{eid}-{lang_key}.mp3")))
            # Dugi
            dugi = p.get("dugi_opis") or p.get("kratki_opis") or ""
            if dugi:
                zadaci.append((dugi, glas, os.path.join(AUDIO_DIR, f"{eid}-{lang_key}-dugi.mp3")))

    print(f"Generiram {len(zadaci)} audio clipova...\n")
    ...
    print(f"\nGotovo! Svi clipovi su u {AUDIO_DIR}/")
